...
const resolvers = {
    ...
    Mutation: {
        addBook: (root, args) => {
            const bookToAdd = { ...args, id: nanoid() }
            const authorExist = authors.find(author => author.name === bookToAdd.author)
            if (!authorExist) {
                const newAuthor = { name: bookToAdd.author, born: null, bookCount: 1, id: nanoid() }
